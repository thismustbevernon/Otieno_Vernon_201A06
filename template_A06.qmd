...
[https://github.com/thismustbevernon/Otieno_Vernon_201_A06](https://github.com/thismustbevernon/Otieno_Vernon_201_A06)
...
for (i in 1 : length(player_info)){
  ...
  points_list[[player_info[[i]][["name"]]]] <-  points_per_game
}
...
```{r}
rounded_points <- sapply(points_list, round , digits=2)
...
```{r}
#use random with -digits
custom_round <- function(num){
  
  if (num < 100){
    return (round(num, digits = -1))
...
apply(mat_rand, 1:2, custom_round)
```
...
max_temps
#rem max_tempsove the first two columns and calculate the average across the rest
avg_per_month <- colMeans(max_temps[, -c(1, 2)], na.rm = TRUE)
...
```{r}
colorado_data <- max_temps %>%
  filter(state=='CO')

ohio_data <- max_temps %>% 
  filter(state == "OH")

virginia_data <- max_temps %>% 
  filter(state=='VA')
...
```{r}
len <- ncol(colorado_data)

yearly_medians <- apply(colorado_data[, 3:len], 1, median)

min_value <- min(yearly_medians)
max_value <- max(yearly_medians)

lowest_year <- colorado_data$year[which.min(yearly_medians)]

highest_year <- colorado_data$year[which.max(yearly_medians)]

print(paste0 ("The maximum median annual temperature of ",max_value," occurred in ",highest_year, " and the minimum median annual temperature of ",min_value, " occurred in ", lowest_year, " ."))
